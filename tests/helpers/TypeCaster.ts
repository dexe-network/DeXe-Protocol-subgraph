...
export function castBlock(block: ethereum.Block): eth.Block {
    return new eth.Block(castBytes(block.hash), castBytes(block.parentHash), castBytes(block.unclesHash), castAddress(block.author), castBytes(block.stateRoot), castBytes(block.transactionsRoot), castBytes(block.receiptsRoot), castBigInt(block.number), castBigInt(block.gasUsed), castBigInt(block.gasLimit), castBigInt(block.timestamp), castBigInt(block.difficulty), castBigInt(block.totalDifficulty), castBigInt(block.size != null ? block.size : BigInt.fromI32(0) ));
}

export function castTransaction(tx: ethereum.Transaction): eth.Transaction {
    return new eth.Transaction(castBytes(tx.hash), castBigInt(tx.index), castAddress(tx.from), castAddress(tx.to != null ? tx.to: Address.zero()), castBigInt(tx.value), castBigInt(tx.gasLimit), castBigInt(tx.gasPrice), castBytes(tx.input));
}
