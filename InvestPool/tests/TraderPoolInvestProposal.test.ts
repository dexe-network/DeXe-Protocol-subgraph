...
describe("TraderPoolInvestProposal", () => {
  ...
  beforeEach(() => {
    let traderPool = getInvestTraderPool(pool);
    traderPool.save();
  ...
  test("should handle ProposalWithdrawn event", () => {
    ...
    assert.fieldEquals("LastWithdrawn", sender.toHexString() + proposalId.toString(), "amountBase", amount.toString());
  });
  ...
  test("should handle ProposalSupplied event", () => {
    ...
    assert.fieldEquals(
      ...
      "[" + amounts[0].toHexString() + ", " + amounts[1].toHexString() + "]"
    );
    assert.fieldEquals(
      "LastSupply",
      sender.toHexString() + proposalId.toString(),
